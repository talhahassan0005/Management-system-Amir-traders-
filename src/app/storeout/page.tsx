...
interface ProductExt { _id: string; item: string; description?: string; brand?: string; width?: number; length?: number; grams?: number; type?: string }
        ...
        (async () => {
          try {
            ...
            setItems(prev => {
              ...
              // Auto-fill qty/weight with current stock so user can reduce
              row2.qty = Number(current) || 0;
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        payloads.push({
          storeId: s._id,
          productId: p._id,
          quantityPkts: -Math.abs(qty),
          weightKg: -Math.abs(weight),
      ...
      for (let i = 0; i < payloads.length; i++) {
        ...
        if (!res.ok) {
          ...
          setErrorMsg(`Row ${i+1} failed: ${err.error || 'Failed to reduce stock'}`);
          return;
        }
      }
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-2 py-2 text-center text-xs font-semibold text-gray-700 uppercase">Qty to Reduce</th>
                        ...
                        <td className="px-2 py-2">
                          {it.baseStock !== undefined ? (
                            <div className="text-xs text-center">
                              ...
                              <div className="text-blue-600">→ {it.stock ?? (it.baseStock - it.qty)}</div>
                            </div>
                          ) : (

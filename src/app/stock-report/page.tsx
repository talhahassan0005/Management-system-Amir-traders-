  ...
  const fetchRows = async () => {
    try {
      ...
      const [stockRes, prodRes, piRes, prRes] = await Promise.all([
        fetch(`/api/stock?limit=1000${qs.replace('&','?')}`),
        fetch(`/api/products?limit=1000`),
        fetch(`/api/purchase-invoices?limit=1000${qs}`),
        fetch(`/api/production?limit=1000${qs}`),
      ]);
      const [stockJson, prodJson, piJson, prJson] = await Promise.all([stockRes.json(), prodRes.json(), piRes.json(), prRes.json()]);
      const stock: any[] = stockRes.ok ? (stockJson.stockData || []) : [];
      ...
      const merged: MergedRow[] = stock.map(s => {
        ...
        return {
          ...
          type: p.type,
          constant: p.constant,
          packets: s.currentStock,
          weightKg: s.stockWeightKg,
            ...
            <table className="min-w-full border border-gray-300 print-table">
              ...
              <tbody>
                {loading ? (
                  ...
                  filtered.map((r, idx) => (
                    <tr key={r._id || r.itemCode} className="odd:bg-white even:bg-gray-50 hover:bg-blue-50">

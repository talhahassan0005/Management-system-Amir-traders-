...
export default function CashSalesValue() {
  ...
  const handleDownload = () => {
    downloadCSV(rows || [], 'cash-sales-value');
  };

  useEffect(() => {
    const q = new URLSearchParams(filters as any).toString();
    setLoading(true);
    fetch(`/api/reports/cash-sales-value${q ? `?${q}` : ''}`)
      .then(r => r.json())
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Cash Sales Value</h1>
            <p className="text-gray-600">Total value of cash sales</p>
          </div>
        ...
        <Filter
          onChange={setFilters}
          show={{ product: true, store: true, customer: true, from: true, to: true, saleType: false }}
            ...
            <table className="min-w-full divide-y divide-gray-200">
                ...
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-sm text-gray-500">
                      No cash sales data available for selected period
                    </td>
                  </tr>
                ) : (
              ...
              {rows.length > 0 && (
                <tfoot className="bg-gray-50">
                  <tr>
                    <td className="px-6 py-3 text-sm font-semibold text-gray-900" colSpan={3}>Total</td>

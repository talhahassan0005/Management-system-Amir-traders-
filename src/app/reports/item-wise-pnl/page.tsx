...
export default function ItemWisePnL() {
  ...
  const handleDownload = () => {
    downloadCSV(rows || [], 'item-wise-pnl');
  };

  useEffect(() => {
    const q = new URLSearchParams(filters as any).toString();
    fetch(`/api/reports/item-wise-pnl${q ? `?${q}` : ''}`)
      .then(r => r.json())
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Item-wise P&L</h1>
            <p className="text-gray-600">Profit & Loss per item</p>
          </div>
  ...
  <Filter onChange={setFilters} show={{ customer: false }} />
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Sales Revenue</th>
                ...
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-8 text-center text-sm text-gray-500">
                      No item-wise profit data available for selected period
                    </td>
                  </tr>
                ) : (
                  rows.map((row: any, index: number) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                        {row.unitsSold != null ? row.unitsSold.toLocaleString() : '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
                        {row.salesRevenue != null ? row.salesRevenue.toLocaleString() : '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">
